	...
	<div class="col">
		<div class="instructions">
			...
			<p>
				...
				<?php _e( 'If you need to change your room you can do it in this box.', 'book-a-room' ); ?>
			</p>
		</div>
		<div class="options">
			<span class="header">
	...
	<div class="col">
		<div class="instructions">
			...
			<p>
				...
				<?php _E( 'Once you have chosen a room, then choose a date.  The hours that are available will be shown below.', 'book-a-room' ); ?><br/>
			</p>
		</div>
		<?php
		# small calendar
				...
				if( $closings !== false ) {
					...
					switch( $incrementList[$i]['type'] ) {
						...
						case 'reserved':
							# Reserved
							?>
							<tr class="calHoursReserved" style="background: get_option( 'bookaroom_reservedColor' ); color: #reservedFont#">
								<td class="calCheckBox"><input id="hours_<?php echo $i; ?>" name="hours[]" type="hidden" value="" onchange="checkHours()"/>
								</td>
								<td class="calTime"><?php echo date( 'g:i a', $curStart ) .' - '. date( 'g:i a', $curEnd ); ?></td>
