...
<table class="tableMain freeWidth">
	...
	foreach ( $closings[ 'times' ][ 'live' ] as $key => $val ) {
		...
		switch ( $closings[ 'live' ][ $key ][ 'type' ] ) {
			case 'range':
				$date = date( $dateDesc, $closings[ 'live' ][ $key ][ 'startTime' ] ) . ' - <br />' . date( $dateDesc, $closings[ 'live' ][ $key ][ 'endTime' ] );
				break;
			case 'date':
				$date = date( $dateDesc, $closings[ 'live' ][ $key ][ 'startTime' ] );
				break;
			default:
		...
		<table class="tableMain freeWidth">
			...
			foreach ( $closings['times']['expired'] as $key => $val ) {
				switch( $closings[ 'expired' ][$key]['type'] ) {
					case 'range':						
						$date = date( $dateDesc, $closings[ 'expired' ][$key]['startTime'] ) . ' - <br />' . date( $dateDesc, $closings[ 'expired' ][$key]['endTime'] );
						break;
					case 'date':
						$date = date( $dateDesc, $closings[ 'expired' ][$key]['startTime'] );
						break;						
					default:
